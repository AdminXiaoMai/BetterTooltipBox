...
public class TooltipHelper {
    ...
    public static void DrawTooltip(int x, int y, int width, int height) {
        // 检测是否在 NEI 环境中
        if (isNEIEnvironment()) {
            renderNEIBackground(x, y, width, height);
        } else {
            renderCustomTooltip(x, y, width, height);
        }
    }

    private static void renderCustomTooltip(int x, int y, int width, int height) {
        GL11.glPushMatrix();
    ...
    public static void DrawNEITooltip(int x, int y, int width, int height) {
        // 调整位置和尺寸以匹配 NEI 的布局
        int adjustedX = x - 3;
        int adjustedY = y - 3;
        int adjustedWidth = width + 6;
        int adjustedHeight = height + 6;

        DrawTooltip(adjustedX, adjustedY, adjustedWidth, adjustedHeight);
    }
    ...
    private static void renderNEIBackground(int x, int y, int width, int height) {
        // 简化的背景渲染
        GL11.glPushMatrix();
        GL11.glEnable(GL11.GL_BLEND);
        Tessellator tessellator = Tessellator.instance;
        ...
        renderQuad(tessellator, x, y, width, height, 0, 0, 64, 64, 64, 64);
